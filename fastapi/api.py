    ...
    try:
        ...
        try:
            ...
            if isinstance(data, dict):
                ...
                if isinstance(t_input, list): transcripts_data = t_input
                elif isinstance(t_input, str): transcripts_data = [{"transcript": t_input, "transcript_id": 0}]
                
                sc_input = data.get("scorecards") or data.get("scorecard")
                if isinstance(sc_input, list): all_scorecards = sc_input
                elif isinstance(sc_input, dict): all_scorecards = [sc_input]
        except:
        ...
        db.hset(f"job:{job_id}", mapping={
            "status": "processing", 
            "total": total_tasks,
            "callback_url": external_api or "",
            "callback_token": external_token or ""
        })
        ...
        for t_idx, t_obj in enumerate(transcripts_data):
            ...
            for s_idx, sc in enumerate(all_scorecards):
                ...
                curr_questions = raw_qs if isinstance(raw_qs, list) else ([raw_qs] if raw_qs else (sc if isinstance(sc, list) else [sc]))

                task_id = f"j{job_id}_t{t_id}_s{sc_id}"
                payload = {
                    "job_id": job_id, "transcript_id": t_id, "scorecard_id": sc_id,
                    "transcript": clean_text, "questions": curr_questions, "task_id": task_id
                }
